...
|   
    expression T_SPACE expression
    {
        if($1.typ != integer || $3.typ != integer)
        {
            semantic_error(@1.begin.line, "SPACE Type error.");
        }

        std::string ifbigger_label = next_label();
        std::string ifless_label = next_label();
